...
The `finalOutput` field is optional. Passing it makes the final agent response visible on `/session/{sessionId}` without digging through the timeline.
...
Open `/session/{sessionId}` to review the session timeline, terminal output, diffs, test results, final output, chapters, notes, and Needs Review markers.
